...
class GrabCut:
    ...
    step 1
    '''
    def convert_rect_to_mask(rect, img):
        mask = np.zeros((img.shape[0], img.shape[1]))
        mask[rect[1]:rect[1]+rect[3], rect[0]:rect[0]+rect[2]] = 2
        return mask

    def convert_rect_to_matte(rect, img):
    ...
    def set_bgd_fgd(self):
        ...
        self.bgd_pixels = img[self.bgd]
        self.fgd_pixels = img[self.fgd]
    ...
    def get_beta(self):
        ...
        sum_squared = (self._left_diff*self._left_diff).sum() + (self._upleft_diff*self._upleft_diff).sum() + (self._up_diff*self._up_diff).sum() + (self._upright_diff*self._upright_diff).sum()
    ...
    def build_n_link(self, nodeids):
        ...
        for y in range(self.height):
            for x in range(self.width):
                Zm = self.img[y][x]
                if y > 0 and x > 0:
                    diag_left[y][x] = 50 / np.sqrt(2) * np.exp(
                        -beta*(Zm - self.img[y-1][x-1])**2)
                if y > 0 and x < self.img.shape[1] - 1:
                    diag_right[y][x] = 50/ np.sqrt(2) * np.exp(
                        -beta*(Zm - self.img[y-1][x+1])**2)
                if x > 0:
                    left[y][x] = 50 * np.exp(
                        -beta*(Zm - self.img[y][x-1])**2)
                if y > 0:
                    up[y][x] = 50 * np.exp(
                        -beta*(Zm - self.img[y-1][x])**2) 

        self.max_weight = max(max(diag_left), max(diag_right), max(left), max(up))
    ...
    '''
    def grabCut(self, img, mask, rect, background, foreground, iteration, use_mask):
        if not use_mask:
        ...
        nodeids = self.graph.add_grid_nodes((self.img.shape[0], self.img.shape[1]))
        self.graph.maxflow()
